...
class QaqcMainModel(IsbitClassifierModel):
    ...
    def dim_red(self, embeddings: Tensor, dim: str | None) -> pd.DataFrame:
        ...
        match dim:
            case "COMBO":
                ...
                tsne = TSNE(
                    n_components=2, random_state=42, perplexity=30, n_iter=300, early_exaggeration=4, learning_rate=1000
                )
            ...
            case "UMAP":
                umap_model = umap.UMAP(
                    n_components=2, random_state=42, n_neighbors=15, min_dist=0.1, metric="euclidean"
                )
    ...
    def latter_run(self, df: pd.DataFrame, dim: str | None) -> pd.DataFrame:
        questions = df["text"].tolist()
        embeddings_tensor = self.get_embeddings(
            questions
        )  # This is equal to the first embeddings(1.2 in our figure), the alternative
        ...
        x_and_y = self.dim_red(
            embeddings=new_embeddings, dim=dim
        )  # generates coordinates x and y for plotting in frontend
